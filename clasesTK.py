...
class Formulario_solicitar(Interfaz):
    ...
    def agregarUsuario(self, id_usuario):
        consultaInsert("INSERT INTO `usuarios_grupos`(`id_grupo`, `id_usuario`) VALUES (%s, %s)", tuple(self.id_grupo, id_usuario))

    def eliminarUsuario(self, id_usuario):
        consultaInsert("UPDATE `usuarios_grupos` SET `estado`='expulsado' WHERE id_usuario = %s", tuple(id_usuario))
